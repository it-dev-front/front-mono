...
const MvpBadge = ({ isMvp }: MvpBadgeProps) => {
  ...
  return (
    <div className="flex items-center justify-center absolute top-0 w-[43px] h-[25px] mobile:w-[30px] mobile:h-[18px] bg-[#ABEE02] text-[#000000] rounded-sm z-1">
...
const SeasonBadge = ({ seasonId }: SeasonBadgeProps) => {
  ...
  return (
    <div className="relative w-[40px] h-[25px] rounded-[4px] overflow-hidden">
      <Image
...
const GradeBadge = ({ spGrade }: GradeBadgeProps) => {
  ...
  const baseStyle =
    "flex items-center justify-center w-[25px] h-[25px] rounded-[4px]";
